...
//             if (!m_pool.empty())
//             {
//                 obj = std::move(m_pool.top()); // 获取可用对象
//                 m_pool.pop();
//             }
...
//                 {
//                     m_pool.push(std::move(obj)); // 将对象放回对象池
//                 }
//             }
//         }
//         else
...
//             {
//                 std::lock_guard<std::mutex> lock(m_mutex);                     // 加锁保护延迟队列
...
//             {
//                 expiredObjects.push_back(std::move(m_delayedQueue.top().obj)); // 获取已到期的对象
//                 m_delayedQueue.pop();
//             }
//         }
...
// // 多线程对象池获取和释放对象
// static void BM_ObjectPoolAcquireReleaseMultiThread(benchmark::State &state)
// {
//     objectpool::ObjectPool<TestObject, int> pool(100, 1000, nullptr, nullptr, nullptr, 42);
//     int num_threads = std::thread::hardware_concurrency();
//     std::vector<std::thread> threads;

//     auto thread_func = [&pool](int iterations)
//     {
...
//         state.PauseTiming();
//         threads.clear();
//         int iterations_per_thread = state.range(0) / num_threads;
//         for (int i = 0; i < num_threads; ++i)
//         {
//             threads.emplace_back(thread_func, iterations_per_thread);
//         }
//         state.ResumeTiming();
...
//     }
// }
// BENCHMARK(BM_ObjectPoolAcquireReleaseMultiThread)->Arg(10000);

// // 多线程直接创建和销毁对象
// static void BM_DirectCreateDestroyMultiThread(benchmark::State &state)
// {
//     int num_threads = std::thread::hardware_concurrency();
//     std::vector<std::thread> threads;

//     auto thread_func = [](int iterations)
//     {
//         for (int i = 0; i < iterations; ++i)
//         {
...
//     }
// }
// BENCHMARK(BM_DirectCreateDestroyMultiThread)->Arg(10000);
